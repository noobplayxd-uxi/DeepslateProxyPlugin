...
public class DeepslateProxyPlugin extends Plugin implements Listener {
                ...
                public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
                    if (msg instanceof ByteBuf) {
                        ...
                        try {
                            ...
                            if (packetId == 0x22) { // Chunk packet for 1.20.1
    ...
    private void processModernChunk(ProxiedPlayer player, ByteBuf buf) throws IOException {
        ...
        for (int sectionY = -4; sectionY < 20; sectionY++) {
            if ((bitmask & (1 << (sectionY + 4))) != 0) {
                ...
                // Convert modern palette to 1.12.2 char array
    ...
    // ===== VarInt, NBT helpers (exactly as in your current file) =====
    private static int readVarInt(ByteBuf buf) { /* ... same as before ... */ }
    private static void writeVarInt(DataOutputStream out, int value) throws IOException { /* ... */ }
    private void skipNBT(ByteBuf buf) { skipCompound(buf); }
